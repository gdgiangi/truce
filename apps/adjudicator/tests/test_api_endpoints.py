...
from truce_adjudicator.models import (
    Claim, ClaimCreate, Evidence, ModelAssessment, VerdictType, 
    ConsensusStatement, VoteType, TimeWindow
)
...
class TestClaimsEndpoints:
    ...
    def test_add_statcan_evidence(self, client, sample_claim_data):
        ...
        with patch('truce_adjudicator.statcan.fetch_csi.fetch_crime_severity_data') as mock_fetch:
            ...
            assert any("Statistics Canada" in e["publisher"] for e in claim_data["evidence"])
...
class TestVerificationEndpoint:
    ...
    async def test_verify_claim_success(self, client, sample_claim_data):
        ...
        with patch('truce_adjudicator.panel.run_panel.create_mock_assessments') as mock_panel:
...
class TestConsensusEndpoints:
    ...
    def test_create_consensus_statement(self, client):
        ...
        response = client.post("/consensus/canada-crime/statements", json=statement_data)
        assert response.status_code == 200
    ...
    def test_vote_on_statement(self, client):
        ...
        create_response = client.post("/consensus/canada-crime/statements", json=statement_data)
    ...
    def test_get_consensus_summary(self, client):
        ...
        for i in range(3):
            statement_data = {
                "text": f"Test statement {i}",
                "topic": "test-topic"
            }
            create_response = client.post("/consensus/test-topic/statements", json=statement_data)
            statement_id = create_response.json()["id"]
