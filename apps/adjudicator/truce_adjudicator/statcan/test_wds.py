...
from truce_adjudicator.statcan.fetch_csi import (
    StatCanWDSClient,
    fetch_crime_severity_data,
)
from truce_adjudicator.statcan.utils import (
    cansim_to_pid,
    get_table_url,
    parse_wds_response,
)
    ...
    try:
        if pid:
            ...
            if parsed["status"] == "SUCCESS":
                ...
                print(
                    f"   ❌ Failed to retrieve metadata: {parsed.get('error', 'Unknown error')}"
                )
    ...
    try:
        ...
        if changed_series:
            ...
            if len(changed_series) > 0:
                print(
                    f"   Example series: Vector {changed_series[0].get('vectorId', 'N/A')}"
                )
        else:
