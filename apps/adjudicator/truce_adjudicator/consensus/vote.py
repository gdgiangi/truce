...
def detect_duplicate_votes(votes: List[Vote]) -> List[Vote]:
    ...
    for vote in votes:
        ...
        identifier = (
            vote.statement_id,
            vote.user_id or vote.session_id
        )
...
def get_consensus_statements(
    ...
    return sorted(consensus, key=lambda s: (s.agree_rate, s.agree_count + s.disagree_count), reverse=True)
...
def get_divisive_statements(
    ...
    for statement in statements:
        ...
        if (total_votes >= min_votes and 
            divisive_range[0] <= statement.agree_rate <= divisive_range[1]):
...
def create_vote_matrix(statements: List[ConsensusStatement], votes: List[Vote]) -> Tuple[np.ndarray, List[str], List[UUID]]:
    """Create user-statement vote matrix for clustering"""
    
    # Get unique users/sessions
    users = list(set(v.user_id or v.session_id for v in votes if v.user_id or v.session_id))
...
def cluster_users_by_votes(
    ...
    try:
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_users = [users[i] for i in range(len(users)) if cluster_labels[i] == cluster_id]
                ...
                for i in range(len(cluster_votes)):
                    for j in range(i + 1, len(cluster_votes)):
                        ...
                        if np.any(both_voted):
                            agreement = np.mean(user_i_votes[both_voted] == user_j_votes[both_voted])
