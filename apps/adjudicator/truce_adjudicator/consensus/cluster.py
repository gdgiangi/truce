...
from ..models import ConsensusStatement, Vote, VoteType, ConsensusCluster
...
def analyze_statement_clusters(
    ...
    votes: List[Vote]
) -> Tuple[List[ConsensusStatement], List[ConsensusStatement], List[ConsensusStatement]]:
    ...
    uncertain_statements.sort(key=lambda s: s.agree_count + s.disagree_count, reverse=True)
...
def find_opinion_bridges(
    ...
    for statement in statements:
        ...
        if len(agree_votes) >= 0.4 * len(statement_votes) and len(agree_votes) <= 0.8 * len(statement_votes):
...
def calculate_consensus_quality_metrics(
    ...
    consensus_statements = [s for s in statements if s.agree_rate >= 0.7 or s.agree_rate <= 0.3]
    metrics["consensus_ratio"] = len(consensus_statements) / len(statements) if statements else 0
    ...
    metrics["polarization_score"] = 1 - np.mean(polarization_scores) if polarization_scores else 0
    
    # Statement coverage - how well distributed votes are across statements
    statement_vote_counts = [statement.agree_count + statement.disagree_count for statement in statements]
...
def suggest_new_statements(
    existing_statements: List[ConsensusStatement],
    votes: List[Vote],
    topic: str
) -> List[str]:
    ...
    consensus_statements, divisive_statements, uncertain_statements = analyze_statement_clusters(
        existing_statements, votes
    )
    ...
    if "crime" in topic.lower() and "canada" in topic.lower():
        ...
        for suggestion in base_suggestions:
            ...
            for existing in existing_statements:
                ...
                common_words = set(suggestion.lower().split()) & set(existing.text.lower().split())
...
def detect_voting_patterns(votes: List[Vote]) -> Dict[str, any]:
    ...
    for user_key, user_vote_list in user_votes.items():
        ...
        for i in range(1, len(user_vote_list)):
            diff = (user_vote_list[i].created_at - user_vote_list[i-1].created_at).total_seconds()
            time_diffs.append(diff)
        
        if time_diffs and np.mean(time_diffs) < 30:  # Average < 30 seconds between votes
