...
class AgenticResearcher:
    ...
    async def conduct_research(self, 
                             ...
                             session_id: Optional[str] = None) -> List[Evidence]:
        """
        ...
        try:
            ...
            async with client:
                ...
                if session_id:
                    await self._emit_progress(session_id, 
                ...
                for turn in range(self.max_search_turns):
                    ...
                    research_plan = await self._update_research_plan(research_plan, analysis)
                    
                    # Check if research is sufficient
                    if analysis.get("sufficient_evidence", False) and len(self.collected_sources) >= 5:
                        if session_id:
                            await self._emit_progress(session_id,
    ...
    async def _plan_research(self, claim: Claim, client: Client) -> Dict[str, Any]:
        ...
        return {
            ...
            "gaps_identified": [],
            "next_actions": ["broad_search", "perspective_search", "targeted_verification"],
        }
    
    async def _execute_research_turn(self, 
                                   claim: Claim, 
                                   client: Client, 
                                   turn: int,
                                   research_plan: Dict[str, Any],
                                   time_window: Optional[TimeWindow]) -> List[Dict[str, Any]]:
        ...
        try:
            if turn == 0:
                ...
                result = await client.call_tool("search_multiple_perspectives", {
                    "claim": claim.text,
                    "perspectives": [
                ...
                result = await client.call_tool("targeted_source_search", {
                    "query": claim.text,
                    "source_types": [
                        "site:statcan.gc.ca",
                        "site:canada.ca", 
    ...
    async def _analyze_turn_results(self, turn_sources: List[Dict[str, Any]], claim: Claim) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "needs_official_sources": not any("gov" in s.get("domain", "") or "statcan" in s.get("domain", "") 
                                           for s in self.collected_sources),
        }
        
        return analysis
    
    async def _update_research_plan(self, current_plan: Dict[str, Any], analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _identify_research_gap(self, claim: Claim, research_plan: Dict[str, Any]) -> str:
        ...
        for source in self.collected_sources:
            try:
                ...
                evidence = Evidence(
                    ...
                    retrieved_at=datetime.fromisoformat(source.get("retrieved_at", datetime.now().isoformat())),
