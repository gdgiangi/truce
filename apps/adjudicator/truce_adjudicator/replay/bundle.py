...
async def _create_jsonld_graph(claim: Claim) -> Dict[str, Any]:
    ...
    if claim.entities:
        claim_obj["tr:entities"] = [{"@id": f"wd:{entity}"} for entity in claim.entities]
    ...
    for assessment in claim.model_assessments:
        assessment_obj = {
            ...
            "tr:citations": [{"@id": f"tr:evidence/{cid}"} for cid in assessment.citations]
        }
...
async def save_replay_bundle(bundle: ReplayBundle, output_dir: str = "data/replay") -> str:
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        for i, (prompt, response) in enumerate(zip(bundle.model_prompts, bundle.model_responses)):
            f.write(json.dumps({
                "type": "model_prompt",
                "sequence": i,
                "data": prompt
